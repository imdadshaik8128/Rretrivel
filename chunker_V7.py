...
def chunk_markdown(filepath: str, 
    ...
    for line_num, line in merged_lines:
        ...
        if m := H1.match(line):
            ...
            sub = m.group(1).strip()
            h3_ctype, h3_stype = classify_section_type(sub) # Check if it's an activity

            # FORCE split if it's an activity, OR if the current segment is large enough
            if current and (h3_ctype == "activity" or token_count("\n".join(current["lines"])) >= min_tokens):
                ...
                # Still a small non-activity segment: merge into current
                if current:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_dir",  default="./subjects_v2",
