...
EQ_RE       = re.compile(r'\$[^$]+\$|\\\[.+?\\\]')
ACTIVITY_RE     = re.compile(r'\bactivity\s*[\d.]+', re.IGNORECASE)
ACTIVITY_NUM_RE = re.compile(r'\bactivity\s*([\d.]+)', re.IGNORECASE)
...
# Activity heading patterns — these produce chunk_type="activity", NOT "exercise"
# Matched against H2/H3 heading text
ACTIVITY_HEADING_RE = re.compile(
    r'^\s*activity\s*[\d.]*\s*$|^\s*activity\s*[\d.]+',
    re.IGNORECASE
)

# Each entry: (section_type_label, [heading phrases that map to it])
# Matched against the lowercase section/subsection heading.
# NOTE: "activity" is intentionally absent — handled separately above.
...
# Flat regex: (section_type_label, compiled pattern)
...
def classify_section_type(heading: str) -> tuple[str, str]:
    """
    Given a heading string (section / subsection),
    return (chunk_type, section_type_label).

    chunk_type   : "activity" | "exercise" | "theory"
    section_type : specific label like "fill_in_the_blanks", "activity", or ""

    Priority order:
      1. Activity headings  → chunk_type="activity"  (NOT exercise)
      2. Exercise headings  → chunk_type="exercise"
      3. Everything else    → chunk_type="theory"
    """
    h = heading.lower().strip()

    # Check activity first — takes priority over exercise patterns
    if ACTIVITY_HEADING_RE.search(heading):
        return "activity", "activity"

    # Check exercise section map
